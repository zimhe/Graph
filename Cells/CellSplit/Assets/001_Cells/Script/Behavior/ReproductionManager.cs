...
namespace RC3.Unity.TetrahedralGrowth
{
    ...
    public class ReproductionManager : MonoBehaviour
    {
        ...
        [SerializeField] private ScalarField _offsetField;
        [SerializeField] private Cells _cellPfb;
        [SerializeField] private float _thickness = 0.1f;

       List<Cells> _allCells=new List<Cells>();
        ...
        private void Update()
        {
            ...
            {
                _cellSwitch = !_cellSwitch;

                _allCells.ForEach(c=>c.VertexSwitch(_cellSwitch));
            }
        }
        ...
        void GrowCell(Vector3 p,float size)
        {
            var cell = Instantiate(_cellPfb, transform);
        ...
        private Vec3d GetFacePoint(HeMesh3d.Face face, out double distance)
        {
            ...
            // scale the normal
            distance = _offsetField.Evaluate((Vector3)p);
            n *= distance / n.Length;
